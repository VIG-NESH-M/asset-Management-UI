...
export class RestapiService {
  ...
  constructor(private httpClient: HttpClient,private router:Router, private tokenService: TokenService) { 
    this.accessToken = localStorage.getItem("access_token") !== null? localStorage.getItem("access_token"): "";
  }


  public registerAsset(assetDto : Asset) {
    return this.httpClient.post<ApiResponse>(`${this.baseUrl}/assets/save`, assetDto);
  }
  ...
  public getAssetById(assetId:number) {
    return this.httpClient.get<ApiResponse>(`${this.baseUrl}/assets/${assetId}`);
  }

  public updateAsset(assetId: number ,assetDto : Asset) {
    return this.httpClient.put<ApiResponse>(`${this.baseUrl}/assets/${assetId}`, assetDto);
  }

  public deleteAsset(assetId: number ) {
    return this.httpClient.delete<ApiResponse>(`${this.baseUrl}/assets/${assetId}`);
  }

  public getAssetsByFacility(facility: string) {
    return this.httpClient.get<ApiResponse>(`${this.baseUrl}/assets/facilities/${facility}`);
  }
  ...
  public addAssetByCsv(file : any) {
    ...
    return this.httpClient.post<ApiResponse>(`${this.baseUrl}/assets/csv`, file, {headers:{
  ...
  public login(loginRequest : LoginRequest) {
    return this.httpClient.post<ApiResponse>(`${this.baseUrl}/authenticate/login`, loginRequest);
  }

  public refreshLogin(refreshRequest : LoginRefreshReq) {
    return this.httpClient.post<ApiResponse>(`${this.baseUrl}/authenticate/login`, refreshRequest);
  }
  ...
  public registerEmployee(employee : Employee) {
    return this.httpClient.post<ApiResponse>(`${this.baseUrl}/employees/save`, employee);
  }
  ...
  public getEmployeeById(empId:number) {
    return this.httpClient.get<ApiResponse>(`${this.baseUrl}/employees/${empId}`);
  }

  public updateEmployee(empId: number ,employee : Employee) {
    return this.httpClient.put<ApiResponse>(`${this.baseUrl}/employees/${empId}`, employee);
  }
  ...
  public createRepo(asset_name : String, file : File, userId  :any) {
    var formData = new FormData();
  ...
  public getAllTicketsByStatus(status: string){
    return  this.httpClient.get<ApiResponse>(`${this.baseUrl}/tickets/list/status/${status}`);
  }

  public updateTicketStatus(ticketId:UpdateStatusReq){
    return  this.httpClient.put<ApiResponse>(`${this.baseUrl}/tickets/status`, ticketId);
  }

  public getAllTicketsEmployeeId(empId: number) {
    return this.httpClient.get<ApiResponse>(`${this.baseUrl}/tickets/list/employees/${empId}`);
  }

  public getTicket(ticketId: String) {
    return this.httpClient.get<ApiResponse>(`${this.baseUrl}/tickets/${ticketId}`);
  }
  ...
  public createFacility(facility: Facility) {
    return this.httpClient.post<ApiResponse>(`${this.baseUrl}/facilities/save`, facility);
  }

  public deleteFacility(facilityId: number) {
    return this.httpClient.delete<ApiResponse>(`${this.baseUrl}/facilities/${facilityId}`);
  }
